...
const themify = require('../../internals/dato-themes');
...
gulp.task('polish:clean:release', () => {
  return del([path.join(distDir, releaseDirName)], { force: true });
});

gulp.task('default', ['build']);

gulp.task('themify', () => {
  return gulp.src('../src/scss/themes/index.themes.scss')
             .pipe(postcss([
               sass(),
               autoprefixer(),
               themify()
...
gulp.task('watch:themes', function() {
  ...
  const path = '../src/scss/themes/index.themes.scss';

  return watch(glob, { ignoreInitial: false }, function() {
    console.log('Build themes ', new Date());
    gulp.src(path)
        .pipe(postcss([
          sass(),
          autoprefixer(),
